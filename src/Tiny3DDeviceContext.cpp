...
Tiny3DDeviceContext::Tiny3DDeviceContext():m_renderMode(TINY3D_FILL_WIREFRAME),m_cameraPos(XVector(0.f,0.f,0.f,1.f))
{
}
...
//设置相机位置
void Tiny3DDeviceContext::SetCameraPos(const XVector& pos)
{
	m_cameraPos = pos;
}
...
{
	//得到屏幕变换矩阵
    XMatrix screenTransformMat = MathUtil::XMatrixScreenTransform(m_pDevice->GetClientWidth(),
...
//转换到屏幕坐标
void Tiny3DDeviceContext::TransformToScreen(const XMatrix& m, VertexOut& v)
{
	v.posH = v.posH * m;
}
...
bool Tiny3DDeviceContext::BackFaceCulling(const VertexIn& v1, const VertexIn& v2, const VertexIn& v3)
{
	...
	{
        XVector vector1 = v2.pos - v1.pos;
        XVector vector2 = v3.pos - v2.pos;
		//顶点缓存中顺序为顺时针
		//叉积得到的方向与右手系一致
        XVector normal = vector1.Cross(vector2);

        XVector viewDir = v1.pos - m_cameraPos;
	...
	if (dx > dy)
	{
		...
		for (int i = 0; i <= dx; ++i)
		{
			if(x1 >= 0 && x1 < m_pDevice->GetClientWidth() && y1 >= 0 && y1 < m_pDevice->getClientHeight())
                m_pDevice->DrawPixel(x1, y1, XVector(0.f, 0.f, 0.f,1.f));
			if (error >= 0)
			{
				error -= deltaX;
				y1 += stepy;
